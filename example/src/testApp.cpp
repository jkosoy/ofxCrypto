...
void testApp::setup(){
    ...
    testImage.loadImage(decodedImage);
    
    
    // and now let's re-encode it.
    ofPixels px;
    px.setFromPixels(testImage.getPixels(), testImage.width, testImage.height, testImage.getPixelsRef().getImageType());
    ...
    testImage2.loadImage(ofxCrypto::base64_decode_to_buffer(reencodedImageData));
}
...
void testApp::draw(){
    ...
    if(testImage2.isAllocated()) {
        testImage2.draw(0,testImage.height + 5);
    }
}
